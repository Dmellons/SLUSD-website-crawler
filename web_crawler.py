...
class SLUSDCrawler:
    def __init__(self, base_url="https://www.slusd.us"):
        ...
        self.found_pages = []
        self.found_pdfs = []
        self.target_address = "835 E. 14th St"
    ...
    def init_csv_file(self):
        ...
        if not os.path.exists(self.output_file):
            with open(self.output_file, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['URL', 'Type', 'Title', 'Address Found', 'Notes', 'Timestamp']
    ...
    def check_address_in_text(self, text):
        """Check if the target address is mentioned in text"""
        try:
            # Case-insensitive search for the address
            return self.target_address.lower() in text.lower()
        except Exception:
            return False
    ...
    def check_pdf_content(self, url):
        ...
        try:
            ...
            return self.check_address_in_text(text_content)
            
        except Exception as e:
            print(f"Error reading PDF {url}: {e}")
            return False

    def record_finding(self, url, content_type, title="", notes=""):
        ...
        result = {
            ...
            'Type': content_type,
            'Title': title,
            'Address Found': self.target_address,
            'Notes': notes,
            'Timestamp': timestamp
        }
        ...
        print(f"✓ Found address in {content_type}: {url}")

    def crawl_site(self, max_pages=5000):
        """Main crawling function with improved error handling"""
        print(f"Starting crawl of {self.base_url}")
        print(f"Looking for pages and PDFs containing: {self.target_address}")
        ...
        while url_queue and pages_crawled < max_pages:
            ...
            try:
                ...
                if content_type and 'pdf' in content_type.lower():
                    if self.check_pdf_content(current_url):
                        self.record_finding(current_url, 'PDF', notes='PDF document')
                    continue
                ...
                # Check if page contains the target address
                if self.check_address_in_text(page_text):
                    title = soup.title.string if soup.title else 'No title'
                    self.record_finding(current_url, 'HTML Page', title=title, notes='HTML page content')
                ...
                for link in links:
                    if (self.is_valid_url(link) and 
                        ...
                        if link.lower().endswith('.pdf'):
                            if self.check_pdf_content(link):
                                self.record_finding(link, 'PDF', notes='PDF document')
                        else:
    ...
    def print_summary(self):
        ...
        if self.found_pages:
            print(f"\nHTML Pages containing '{self.target_address}':")
        ...
        if self.found_pdfs:
            print(f"\nPDFs containing '{self.target_address}':")
